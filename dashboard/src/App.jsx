...
function App() {
  ...
  const getComplianceClass = (status) => {
    if (status === "NON-COMPLIANT") return "compliance-bad";
    if (status === "AT RISK") return "compliance-warn";
    return "compliance-good";
  };

  const metrics = useMemo(() => {
    const total = history.length;
    const high = history.filter((item) => item.predicted_degradation_risk === "High").length;
    const moderate = history.filter((item) => item.predicted_degradation_risk === "Moderate").length;
    const low = history.filter((item) => item.predicted_degradation_risk === "Low").length;
    const sensor = history.filter((item) => item.source === "sensor").length;
    const manual = history.filter((item) => item.source === "manual").length;
    ...
    return { total, high, moderate, low, sensor, manual, drugCounts };
  }, [history]);
  ...
  return (
    <div className="container">
      ...
      <div className="status-bar">
        ...
        <div className="status-item">
          <strong>Last Refresh:</strong> {lastRefresh || "Waiting..."}
        </div>
        <div className="status-item">
          <strong>Monitoring Mode:</strong> Live + Manual
        </div>
      </div>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card high-card">
          ...
          <span className="stat-value">{metrics.high}</span>
        </div>
        <div className="stat-card moderate-card">
          <span className="stat-label">Moderate Risk</span>
          <span className="stat-value">{metrics.moderate}</span>
        </div>
        <div className="stat-card low-card">
          <span className="stat-label">Low Risk</span>
          <span className="stat-value">{metrics.low}</span>
        </div>
        <div className="stat-card">
          <span className="stat-label">Sensor Events</span>
          <span className="stat-value">{metrics.sensor}</span>
        </div>
        <div className="stat-card">
          <span className="stat-label">Manual Events</span>
          <span className="stat-value">{metrics.manual}</span>
        </div>
      </div>
        ...
        <div className="card">
          ...
          {liveResult ? (
            <div className="result-block">
              ...
              <div className={`compliance-box ${getComplianceClass(liveResult.compliance_status)}`}>
      ...
      {result && (
        <div className="card result-card">
          ...
          <div className={`compliance-box ${getComplianceClass(result.compliance_status)}`}>
          ...
          <div className="chart-group">
            <div className="chart-row">
              ...
              <div className="bar-track">
                <div className="bar-fill bar-high" style={{ width: `${(metrics.high / maxRiskCount) * 100}%` }}></div>
              </div>
              <span className="chart-value">{metrics.high}</span>
            </div>

            <div className="chart-row">
              <span className="chart-label">Moderate</span>
              <div className="bar-track">
                <div className="bar-fill bar-moderate" style={{ width: `${(metrics.moderate / maxRiskCount) * 100}%` }}></div>
              </div>
              <span className="chart-value">{metrics.moderate}</span>
            </div>

            <div className="chart-row">
              <span className="chart-label">Low</span>
              <div className="bar-track">
                <div className="bar-fill bar-low" style={{ width: `${(metrics.low / maxRiskCount) * 100}%` }}></div>
              </div>
              <span className="chart-value">{metrics.low}</span>
            </div>
          </div>
        </div>

        <div className="card">
          <h2>Drug Activity Summary</h2>
          {Object.keys(metrics.drugCounts).length > 0 ? (
            <div className="drug-summary">
              {Object.entries(metrics.drugCounts).slice(0, 6).map(([drug, count]) => (
                <div key={drug} className="drug-summary-row">
                  <span>{drug}</span>
                  <strong>{count}</strong>
                </div>
              ))}
